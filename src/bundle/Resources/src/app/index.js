...
const GeoFinder = () => {
  ...
  return (
    <>
      ...
      <label id="combosearch_label" class="sr-only" >{label}</label>
      <Combobox
      ...
      >
        ...
        {addresses && (
          <ComboboxPopover className="shadow-popup">
            {addresses.length > 0 ? (
              <ComboboxList>
                {addresses.map(({ properties: { id, name, postcode, city,label } }) => (
                  <ComboboxOption key={id} value={label} >{name !== city ? name + ' ' + postcode + ' ' + city : name + ' ' + postcode }</ComboboxOption>
                ))}
              </ComboboxList>
            ) : (
      ...
      <button className="button btn bnt-submit" type="submit" style={{padding: '6px 10px 7px', 'margin': '0 0 0 10px','verticalAlign':'baseline'}} onClick={handleDoSearch} >Ok</button>
